...
class RfqCoreRepositoryImpl(
    ...
    override fun save(rfq: RfqCore): RfqCore {
        jdbcTemplate.update(
            ...
            PreparedStatementArray.setTextArray(ps, i++, rfq.supplierTypes)
            ps.setString(i++, rfq.status.name)
            ps.setString(i++, rfq.buyerCountry)
            if (rfq.categoryId != null) ps.setLong(i++, rfq.categoryId) else ps.setObject(i++, null)
    ...
    override fun updateStatus(rfqId: UUID, status: RfqStatus): Int =
    ...
    private companion object {
        private val ROW_MAPPER = RowMapper { rs, _ ->
            ...
            val supplierTypes = typesArray?.array?.let { arr ->
