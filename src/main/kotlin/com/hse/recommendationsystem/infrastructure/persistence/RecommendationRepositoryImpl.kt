...
class RecommendationRepositoryImpl(
    ...
    override fun save(recommendation: Recommendation) {
        jdbcTemplate.update(
            ...
            ps.setObject(i++, JsonbSupport.toPgObject(objectMapper, recommendation.rawRecommendationJson))
    ...
    override fun deleteAllDataForRfq(rfqId: UUID) {
        ...
        namedParameterJdbcTemplate.update("DELETE FROM recommendations_notifications WHERE rfq_id = :rfqId", params)
        namedParameterJdbcTemplate.update("DELETE FROM decision WHERE rfq_id = :rfqId", params)
        namedParameterJdbcTemplate.update("DELETE FROM supplier_profile_snapshot WHERE rfq_id = :rfqId", params)
        namedParameterJdbcTemplate.update("DELETE FROM recommendation WHERE rfq_id = :rfqId", params)
    }
    ...
    override fun findWithSnapshotsByRfqId(
        ...
        val rows = namedParameterJdbcTemplate.query(
            ...
            LEFT JOIN recommendations_notifications n
              ON n.rfq_id = r.rfq_id AND n.supplier_id = r.supplier_id
            ...
            JoinRowMapper(objectMapper),
        )
        return rows to total
    }

    override fun existsByRfqAndSupplier(rfqId: UUID, supplierId: UUID): Boolean {
    ...
    private class JoinRowMapper(
        private val objectMapper: ObjectMapper,
    ) : RowMapper<RecommendationWithSnapshot> {
        override fun mapRow(rs: java.sql.ResultSet, rowNum: Int): RecommendationWithSnapshot {
            ...
            val recommendation = Recommendation(
                ...
                rawRecommendationJson = JsonbSupport.parseJson(objectMapper, rs.getString("raw_recommendation_json")),
            ...
            val decision = if (decisionTypeStr != null) {
                SupplierDecision(
                    rfqId = rfqId,
                    supplierId = supplierId,
            ...
            val notificationStatusStr = rs.getString("notification_status")
            val notificationStatus = notificationStatusStr?.let { NotificationStatus.valueOf(it) }
        ...
        private fun readTextArray(rs: java.sql.ResultSet, column: String): List<String>? {
